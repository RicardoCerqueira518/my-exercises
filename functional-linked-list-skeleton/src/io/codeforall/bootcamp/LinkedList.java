...
public class LinkedList<T> implements Iterable<T> {
    ...
    public <R> LinkedList<R> map(Function<T, R> mapper) {
        ...
        while (current != null) {
            R transformedData = mapper.apply(current.data); // Aplica a função de transformação
            newList.add(transformedData); // Adiciona o resultado à nova lista
    ...
    public T reduce(BinaryOperator<T> reducer, T accumulator) {
        ...
        while (current != null) {
            accumulator = reducer.apply(accumulator, current.data);
            current = current.next;
        }
